...
if model == 'theta':
    ...
    fixed_params = {'labels': [0,1,2,4,5],
                    'values': vals}
    labels2 = ['bias']
    directory = '.\\results_tmp\\split_seg_theta\\'
    ...
    fixed_params = {'labels': [0,2,3,4,5],
                    'values': vals}
    labels2 = ['beta']
    directory = '.\\results_tmp\\split_seg_beta\\'
    ...
    fixed_params = {'labels': [0,2,4,5],
                    'values': vals}
    labels2 = ['beta','bias']
    directory = '.\\results_tmp\\split_seg_theta_beta\\'
    ...
    directory = '.\\results_tmp\\split_seg_theta_beta_gamma\\'
    ...
    directory = '.\\results_tmp\\split_seg_theta_beta_kappa\\'
    ...
    directory = '.\\results_tmp\\split_seg_theta_beta_gamma_kappa\\'
    
#%% Load and preprocess data
tmp = pd.read_csv('../Results/preprocessed_results.csv')
trials = 15 # number of trials
trials_per_segment = 5 #% number of trials per segment
...
for segment in range(3):
    # Index to select miniblock segment
    idx =  (tmp['phase'] > 1) &  ( (tmp['trial'] > 5*(segment)) & (tmp['trial'] <= 5*(segment+1))   ) 
    ...
    states = torch.tensor(np.ravel_multi_index(trial_values.T, (L, )*2).T.reshape(n_subjects, -1, trials_per_segment))
    point_diff = torch.tensor(tmp.loc[idx,['score_difference']].values.reshape(n_subjects, -1, trials_per_segment), dtype=torch.float)
    offers = torch.tensor(tmp.loc[idx,['offer']].values.reshape(n_subjects, -1, trials_per_segment)-1)
    responses = torch.tensor(2-tmp.loc[idx,['response']].values.reshape(n_subjects, -1, trials_per_segment)) #map response labels to 0-> wait and 1 -> accept 
    
    # exlude trials without clear G-choice classification 
    exclude = ((point_diff == 1) & (offers == 3)) | ((point_diff == -1) & (offers == 2))
    mask = torch.tensor(tmp.loc[idx,['valid']].values.reshape(n_subjects, -1, trials_per_segment), dtype=torch.bool)
